...
async function loadDashboard() {
  ...
  if (!meR.ok) { location.href = 'login.php'; return; }
  ...
  F.$('#recent-outfits').innerHTML = recent.length
    ? recent.map(o => `<a class="outfit-row" href="diva.php?outfit=${encodeURIComponent(o.id)}" style="display:flex;gap:.8rem;align-items:center;padding:.55rem 0;border-bottom:1px solid var(--border)">
        <div class="thumb">✦</div>
        <div class="meta"><b>${o.name}</b><span>${new Date(o.created_at).toLocaleDateString()}</span></div>
      </a>`).join('')
    : `<p class="muted">No saved outfits yet — head to <a style="color:var(--accent)" href="diva.php">Diva Studio</a>.</p>`;
